//! Appchain ("L2") contract for the L1 -> L2 direction.
//!
//! The settlement layer sends a message (via the piltover core's
//! `send_message_to_appchain`) that Katana relays into the `mint_game`
//! `#[l1_handler]`, recording the purchase. The frontend polls the view
//! functions to react to the new on-chain state.
...
mod game_minter {
    ...
    /// Handles a message relayed from the settlement layer. The messaging service
    /// always prepends the settlement-side caller as `from_address`; the remaining
    /// arguments come from the message payload.
    #[l1_handler]
