...
- **Columns come back as hex strings** (`"0x…41"`). Collapse to numbers:
  `const num = (v) => Number(BigInt(v));` `app/src/chain.ts:44`
...
- **Block heights** (appchain tip) and **piltover `get_state`** (settled height) —
  the "settled N / tip M" gauge. `app/src/chain.ts:277`
- **The L1 purchase log** (piltover `MessageSent`) via `getEvents`, so a purchase
  ...
  either world. `app/src/chain.ts:185`
...
- **L1→L2 (buy):** `piltover.send_message_to_appchain(game_system, mint_selector, payload)` — `app/src/chain.ts:141`
- **L2 (play):** `gameSystem.play_game()` — `app/src/chain.ts:249`
- **L2→L1 (bank):** `scoreSystem.claim_score(game_system, player, score)` — `app/src/chain.ts:288`
