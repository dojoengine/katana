...
```
   Starknet Sepolia (remote)                     Local appchain (Katana rollup, :5061)
  ┌─────────────────────────────────┐           ┌──────────────────────────────┐
...
```bash
katana --chain "$CHAIN_DIR" --tee mock --dev --dev.no-fee --http.port 5061 \
...
Port `5061` (and the toriis on `8091`/`8092`, the client on `3002`) are chosen
...
saya-tee tee start --mock-prove \
  --rollup-rpc http://localhost:5061 \
...
torii --rpc http://localhost:5061 --world "$GAME_WORLD" --http.port 8092 ... # appchain
```
