...
use crate::parse::{VersionedEnum, VersionedInput, VersionedKind, VersionedStruct};
use crate::utils;

// Old derive macro functions are no longer needed since we use attribute macro now
...
fn generate_version_module(
    ...
    let fields: Vec<TokenStream> = versioned_struct
        .fields
        .iter()
        .filter_map(|f| {
            ...
            let field_ty = if let Some(version_type) = f.versions.get(version) {
                let ty_path: Path = syn::parse_str(version_type)
                    .unwrap_or_else(|_| panic!("Invalid type path: {}", version_type));
                quote! { #ty_path }
            } else {
                let ty = &f.ty;
                quote! { #ty }
    ...
    // Generate From implementation for version struct to current struct
    // If current_path is specified, use that; otherwise convert to super::StructName
    let target_type = if let Some(ref current_path) = input.current_path {
        utils::type_from_path(current_path, &struct_name.to_string())
    } else {
        // Default to the struct in the parent module (where the user defined it)
        syn::parse_quote!(super::#struct_name)
    };

    let from_impl =
        generate_struct_from_impl(&struct_name, &target_type, versioned_struct, version);
...
fn generate_struct_from_impl(
    ...
    let field_conversions: Vec<TokenStream> = versioned_struct
        .fields
        .iter()
        .filter_map(|f| {
    ...
    let default_fields: Vec<TokenStream> = versioned_struct
        .fields
        .iter()
        .filter_map(|f| {
            if let Some(ref added) = f.added_in {
...
// New functions for attribute macro that include the original struct/enum in the output

pub fn generate_struct_versioned(input: VersionedInput, original: &DeriveInput) -> TokenStream {
    let VersionedKind::Struct(ref versioned_struct) = input.kind else {
        panic!("Expected struct");
    };

    // Create a modified version of the original struct without #[version(...)] attributes
    let mut modified = original.clone();
    if let syn::Data::Struct(ref mut data_struct) = modified.data {
        for field in data_struct.fields.iter_mut() {
            // Remove #[version(...)] attributes from fields
            field.attrs.retain(|attr| !attr.path().is_ident("version"));
        }
    }

    // Include the modified struct definition
    let struct_def = quote! { #modified };

    // Generate version-specific modules based on field attributes
    let version_modules = input
        .versions
        .iter()
        .map(|version| generate_version_module(version, &input, versioned_struct));
...
pub fn generate_enum_versioned(input: VersionedInput, original: &DeriveInput) -> TokenStream {
    let VersionedKind::Enum(ref versioned_enum) = input.kind else {
        panic!("Expected enum");
    };

    // Include the original enum definition
    let enum_def = quote! { #original };
