...
impl<Tx> TrieWriter for DbProvider<Tx>
where
    Tx: DbTxMut + Clone + Send + Sync + 'static,
{
    fn trie_insert_declared_classes(
        ...
        let mut trie = ClassesTrie::new(TrieDbMut::<tables::ClassesTrie, _>::new(self.tx()));
    ...
    fn trie_insert_contract_updates(
        ...
        let mut contract_trie_db =
            ContractsTrie::new(TrieDbMut::<tables::ContractsTrie, _>::new(self.tx()));
        ...
        let leaf_hashes: Vec<_> = {
            ...
            for (address, storage_entries) in &state_updates.storage_updates {
                let mut storage_trie_db = StoragesTrie::new(
                    TrieDbMut::<tables::StoragesTrie, _>::new(self.tx()),
                    *address,
                );
