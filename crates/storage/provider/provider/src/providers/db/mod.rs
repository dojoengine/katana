...
impl<Db: Database> BlockNumberProvider for DbProvider<Db> {
    fn block_number_by_hash(&self, hash: BlockHash) -> ProviderResult<Option<BlockNumber>> {
        self.with_ro(|ro| ro.block_number_by_hash(hash))
    }

    fn latest_number(&self) -> ProviderResult<BlockNumber> {
        self.with_ro(|ro| ro.latest_number())
    }
}
...
impl<Db: Database> BlockHashProvider for DbProvider<Db> {
    fn latest_hash(&self) -> ProviderResult<BlockHash> {
        self.with_ro(|ro| ro.latest_hash())
    }

    fn block_hash_by_num(&self, num: BlockNumber) -> ProviderResult<Option<BlockHash>> {
        self.with_ro(|ro| ro.block_hash_by_num(num))
    }
}
...
impl<Tx: DbTx> BlockStatusProvider for DbROProvider<Tx> {
    fn block_status(&self, id: BlockHashOrNumber) -> ProviderResult<Option<FinalityStatus>> {
        ...
        if let Some(block_num) = block_num {
            let status = self
                .0
                .get::<tables::BlockStatusses>(block_num)?
...
impl<Tx: DbTx> TransactionProvider for DbROProvider<Tx> {
    fn transaction_by_hash(&self, hash: TxHash) -> ProviderResult<Option<TxWithHash>> {
        if let Some(num) = self.0.get::<tables::TxNumbers>(hash)? {
            let transaction =
                self.0.get::<tables::Transactions>(num)?.ok_or(ProviderError::MissingTx(num))?;
    ...
    fn transaction_in_range(&self, range: Range<TxNumber>) -> ProviderResult<Vec<TxWithHash>> {
        ...
        for i in range {
            if let Some(transaction) = self.0.get::<tables::Transactions>(i)? {
                let hash =
                    self.0.get::<tables::TxHashes>(i)?.ok_or(ProviderError::MissingTxHash(i))?;
    ...
    fn transaction_block_num_and_hash(
        ...
        if let Some(num) = self.0.get::<tables::TxNumbers>(hash)? {
            ...
            let block_hash = self
                .0
                .get::<tables::BlockHashes>(block_num)?
...
impl<Db: Database> BlockProvider for DbProvider<Db> {
    fn block_body_indices(
        ...
        self.with_ro(|ro| ro.block_body_indices(id))
    }

    fn block(&self, id: BlockHashOrNumber) -> ProviderResult<Option<Block>> {
        self.with_ro(|ro| ro.block(id))
    }
    ...
    fn block_with_tx_hashes(
        ...
        self.with_ro(|ro| ro.block_with_tx_hashes(id))
    }

    fn blocks_in_range(&self, range: RangeInclusive<u64>) -> ProviderResult<Vec<Block>> {
        self.with_ro(|ro| ro.blocks_in_range(range))
    }
}

impl<Db: Database> BlockStatusProvider for DbProvider<Db> {
    fn block_status(&self, id: BlockHashOrNumber) -> ProviderResult<Option<FinalityStatus>> {
        self.with_ro(|ro| ro.block_status(id))
    }
}
...
impl<Db: Database> StateUpdateProvider for DbProvider<Db> {
    fn state_update(&self, block_id: BlockHashOrNumber) -> ProviderResult<Option<StateUpdates>> {
        self.with_ro(|ro| ro.state_update(block_id))
    }
    ...
    fn declared_classes(
        ...
        self.with_ro(|ro| ro.declared_classes(block_id))
    }
    ...
    fn deployed_contracts(
        ...
        self.with_ro(|ro| ro.deployed_contracts(block_id))
    }
}

impl<Db: Database> TransactionProvider for DbProvider<Db> {
    fn transaction_by_hash(&self, hash: TxHash) -> ProviderResult<Option<TxWithHash>> {
        self.with_ro(|ro| ro.transaction_by_hash(hash))
    }
    ...
    fn transactions_by_block(
        ...
        self.with_ro(|ro| ro.transactions_by_block(block_id))
    }

    fn transaction_in_range(&self, range: Range<TxNumber>) -> ProviderResult<Vec<TxWithHash>> {
        self.with_ro(|ro| ro.transaction_in_range(range))
    }
    ...
    fn transaction_block_num_and_hash(
        ...
        self.with_ro(|ro| ro.transaction_block_num_and_hash(hash))
    }
    ...
    fn transaction_by_block_and_idx(
        ...
        self.with_ro(|ro| ro.transaction_by_block_and_idx(block_id, idx))
    }
    ...
    fn transaction_count_by_block(
        ...
        self.with_ro(|ro| ro.transaction_count_by_block(block_id))
    }
}

impl<Db: Database> TransactionsProviderExt for DbProvider<Db> {
    fn transaction_hashes_in_range(&self, range: Range<TxNumber>) -> ProviderResult<Vec<TxHash>> {
        self.with_ro(|ro| ro.transaction_hashes_in_range(range))
    }

    fn total_transactions(&self) -> ProviderResult<usize> {
        self.with_ro(|ro| ro.total_transactions())
    }
}

impl<Db: Database> TransactionStatusProvider for DbProvider<Db> {
    fn transaction_status(&self, hash: TxHash) -> ProviderResult<Option<FinalityStatus>> {
        self.with_ro(|ro| ro.transaction_status(hash))
    }
}
...
impl<Db: Database> TransactionTraceProvider for DbProvider<Db> {
    fn transaction_execution(
        ...
        self.with_ro(|ro| ro.transaction_execution(hash))
    }
    ...
    fn transaction_executions_by_block(
        ...
        self.with_ro(|ro| ro.transaction_executions_by_block(block_id))
    }
    ...
    fn transaction_executions_in_range(
        ...
        self.with_ro(|ro| ro.transaction_executions_in_range(range))
    }
}

impl<Db: Database> ReceiptProvider for DbProvider<Db> {
    fn receipt_by_hash(&self, hash: TxHash) -> ProviderResult<Option<Receipt>> {
        self.with_ro(|ro| ro.receipt_by_hash(hash))
    }
    ...
    fn receipts_by_block(
        ...
        self.with_ro(|ro| ro.receipts_by_block(block_id))
    }
}

impl<Db: Database> BlockEnvProvider for DbProvider<Db> {
    fn block_env_at(&self, block_id: BlockHashOrNumber) -> ProviderResult<Option<BlockEnv>> {
        self.with_ro(|ro| ro.block_env_at(block_id))
    }
}
...
impl<Db: Database> BlockWriter for DbProvider<Db> {
    fn insert_block_with_states_and_receipts(
        ...
        self.with_rw(|rw| rw.insert_block_with_states_and_receipts(block, states, receipts, executions))
    }
}

impl<Db: Database> StageCheckpointProvider for DbProvider<Db> {
    fn checkpoint(&self, id: &str) -> ProviderResult<Option<BlockNumber>> {
        self.with_ro(|ro| ro.checkpoint(id))
    }

    fn set_checkpoint(&self, id: &str, block_number: BlockNumber) -> ProviderResult<()> {
        self.with_rw(|rw| rw.set_checkpoint(id, block_number))
    }
}
