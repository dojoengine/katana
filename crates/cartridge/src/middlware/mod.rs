...
use crate::vrf::client::VrfClientError;
use crate::CartridgeApiClient;
    ...
    async fn get_vrf_calls(
        ...
        if let Some((req_rand_call, position)) = utils::request_random_call(calls) {
            ...
            if req_rand_call.to != vrf_service.account_address() {
                return Err(Error::Vrf(
                    format!("request_random call must target the vrf account",),
                ));
            }
            ...
            execute_from_outside_call = build_execute_from_outside_call_from_vrf_result(&result);
        }
...
impl Clone for ControllerDeployment {
    fn clone(&self) -> Self {
        Self {
            ...
            vrf_account_address: self.vrf_account_address,
