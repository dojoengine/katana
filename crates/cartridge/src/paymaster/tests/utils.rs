...
use starknet::core::types::{Call as StarknetCall, FlattenedSierraClass};
...
pub async fn deploy_vrf_provider(node: &TestNode, paymaster_address: ContractAddress) {
    ...
    let starknet_client = node.starknet_rpc_client();
...
// crafted outside execution is always a V3.
pub fn assert_crafted_outside_execution(
    ...
    execute_from_outside_selector: Felt,
) {
    assert_eq!(new_outside_execution.caller(), ContractAddress::ZERO);

    if let OutsideExecution::V3(v3) = new_outside_execution {
        // the built outside execution always contains 3 calls:
        //  - VRF submit random
        //  - execute_from_outside_vX (with original outside execution calls)
        //  - VRF assert consumed
        assert_eq!(v3.calls.len(), 3);

        assert_eq!(v3.nonce, NonceChannel::new(Felt::ZERO, 0));
        assert_eq!(v3.caller, ContractAddress::ZERO);
        ...
        // second call should be the original outside execution call.
        assert_eq!(v3.calls[1].to, controller_address);
        assert_eq!(v3.calls[1].selector, execute_from_outside_selector);

        // finally, the last call should be assert_consumed
        assert_eq!(v3.calls[2].to, vrf_address);
        assert_eq!(v3.calls[2].selector, selector!("assert_consumed"));
        assert_eq!(v3.calls[2].calldata.len(), 1);

        if let Some(seed) = seed {
            assert_eq!(v3.calls[2].calldata[0], seed);
        }
    } else {
