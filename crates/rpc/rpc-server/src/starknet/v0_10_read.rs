//! v0.10 Read API implementation.
//!
//! Delegates to the same shared helpers as v0.9, converting results to v0.10 types.
...
use katana_rpc_api::starknet::v0_10::StarknetApiServer;
use katana_rpc_types::block::{BlockHashAndNumberResponse, BlockNumberResponse};
...
use katana_rpc_types::v0_10::event::GetEventsResponse;
    ...
    <PF as ProviderFactory>::Provider: ProviderRO,
{
    ...
    async fn get_block_with_tx_hashes(
        ...
        Ok(self.block_with_tx_hashes(block_id).await?.into())
    }
    ...
    async fn get_block_with_txs(
        ...
        Ok(self.block_with_txs(block_id).await?.into())
    }
    ...
    async fn get_block_with_receipts(
        ...
        Ok(self.block_with_receipts(block_id).await?.into())
    }
    ...
    async fn get_events(
        &self,
        filter: katana_rpc_types::v0_10::event::EventFilterWithPage,
    ) -> RpcResult<GetEventsResponse> {
        // v0.10 EventFilterWithPage re-exports the same type, so we can pass it directly
        // to the shared helper, then convert the response.
        let shared_filter = katana_rpc_types::event::EventFilterWithPage {
            event_filter: filter.event_filter,
            result_page_request: filter.result_page_request,
        };
        Ok(self.events(shared_filter).await?.into())
    }
