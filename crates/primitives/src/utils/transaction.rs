...
pub fn compute_l1_to_l2_message_hash(
    ...
    let from_address = U256::from_be_slice(from_address.0.as_slice());
    ...
    hasher.update(entry_point_selector.to_bytes_be());

    // payload.length: Encode as uint256 (32 bytes)
    let payload_len = U256::from(payload.len());
...
mod tests {
    ...
    #[test]
    fn test_compute_l1_to_l2_message_hash() {
        ...
        let actual_hash = compute_l1_to_l2_message_hash(
            from_address,
            to_address,
            entry_point_selector,
            &payload,
