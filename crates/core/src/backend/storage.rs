...
impl Blockchain {
    ...
    pub async fn new_optimistic(
        ...
        let chain_id =
            starknet_client.chain_id().await.context("failed to fetch forked network id")?;
        ...
        // TODO: convert this to block number instead of BlockHashOrNumber so that it is easier to
        // check if the requested block is within the supported range or not.
        let database = CachedDbProvider::new(db, block_id, starknet_client.clone());
        ...
        {
            let parent_block_id = BlockIdOrTag::from(forked_block.parent_hash);
            let parent_block = starknet_client.get_block_with_tx_hashes(parent_block_id).await?;
