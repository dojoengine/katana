...
impl<P, D> Blocks<P, D> {
    ...
    pub fn new(provider: P, downloader: D) -> Self {
    ...
    D: BlockDownloader,
{
    ...
    async fn execute(&mut self, input: &StageExecutionInput) -> StageResult {
        ...
        let blocks = self.downloader.download_blocks(&block_keys).await.map_err(Error::Gateway)?;
...
/// Gateway-based implementation of [`BlockDownloader`].
///
/// This implementation uses the sequencer gateway to download blocks in batches.
#[derive(Debug)]
pub struct GatewayBlockDownloader {
    inner: BatchDownloader<GatewayBlockFetcher>,
}

impl GatewayBlockDownloader {
    /// Create a new [`GatewayBlockDownloader`] with the given gateway and batch size.
    pub fn new(gateway: SequencerGateway, batch_size: usize) -> Self {
        let fetcher = GatewayBlockFetcher::new(gateway);
        let inner = BatchDownloader::new(fetcher, batch_size);
        Self { inner }
    }
}

impl BlockDownloader for GatewayBlockDownloader {
    fn download_blocks(
        &self,
        block_numbers: &[BlockNumber],
    ) -> impl Future<Output = Result<Vec<StateUpdateWithBlock>, katana_gateway::client::Error>> + Send
    {
        self.inner.download(block_numbers)
    }
}

/// Internal [`Downloader`] implementation that uses the sequencer gateway for downloading a block.
/// This is used by [`GatewayBlockDownloader`].
#[derive(Debug)]
struct GatewayBlockFetcher {
    gateway: SequencerGateway,
}

impl GatewayBlockFetcher {
    fn new(gateway: SequencerGateway) -> Self {
        Self { gateway }
    }
}

impl Downloader for GatewayBlockFetcher {
...
#[cfg(test)]
mod tests {
    //! Unit tests for the [`Blocks`] stage.
    //!
    //! These tests use mock implementations to verify all control flows including
    //! successful downloads, storage operations, and error handling.
    ...
    use super::{BlockDownloader, Blocks, GatewayBlockDownloader};
    ...
    impl BlockDownloader for MockBlockDownloader {
        ...
        ) -> impl Future<Output = Result<Vec<StateUpdateWithBlock>, katana_gateway::client::Error>> + Send
        {
            async move {
                // Track the call
                self.download_calls.lock().unwrap().push(block_numbers.to_vec());
    ...
    impl MockBlockWriter {
        ...
        fn stored_block_numbers(&self) -> Vec<BlockNumber> {
            self.blocks
                ...
                .unwrap()
                .iter()
                .map(|(block, _, _)| block.block.header.number)
                .collect()
        }
    }
    ...
    async fn fetch_blocks_from_gateway() {
        ...
        let downloader = GatewayBlockDownloader::new(feeder_gateway, 10);
