...
/// Trait for accessing the database from a provider.
...
impl<Db: Database> DatabaseProvider for katana_provider::providers::db::DbProvider<Db> {
    type Db = Db;

    fn db(&self) -> &Self::Db {
        katana_provider::providers::db::DbProvider::db(self)
    }
}
...
where
    PF: ProviderFactory,
    <PF as ProviderFactory>::Provider: BlockHashProvider,
    <PF as ProviderFactory>::ProviderMut: BlockWriter + DatabaseProvider + StageCheckpointProvider,
    D: BlockDownloader,
{
    ...
    fn unwind(&mut self, unwind_to: BlockNumber) -> BoxFuture<'_, StageResult> {
        Box::pin(async move {
            ...
            // Unwind blocks
            Self::unwind_blocks(provider_mut.db(), unwind_to)?;
