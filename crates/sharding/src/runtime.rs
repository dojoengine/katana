...
/// # Shutdown
///
/// - [`shutdown_timeout`](Self::shutdown_timeout) — signals the scheduler and
///   joins workers up to a deadline; remaining threads are detached.
/// - [`shutdown_background`](Self::shutdown_background) — signals the scheduler
///   and drops handles so workers detach immediately.
/// - **Drop** — if the runtime has not been consumed by one of the above methods,
///   the `Drop` impl signals the scheduler and blocks until all workers exit.
...
impl ShardRuntime {
    ...
    pub fn new(worker_count: usize, time_quantum: Duration, block_env: Arc<RwLock<BlockEnv>>) -> Self {
