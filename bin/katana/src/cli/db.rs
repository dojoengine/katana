...
fn prune_database(db_path: &str, args: PruneArgs) -> Result<()> {
...
/// Prune the changeset table by removing all entries from the genesis block up to the cutoff block (inclusive).
