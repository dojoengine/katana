...
pub async fn build_single_owner_account(
    ...
    let url = Url::parse(rpc_url)
        .map_err(|e| Error::Starknet(format!("Invalid Starknet RPC URL: {e}")))?;
    ...
    let chain_id = provider
        .chain_id()
        .await
        .map_err(|e| Error::Starknet(format!("Failed to fetch chain_id: {e}")))?;
...
impl KatanaTeeStarknetClient {
    ...
    pub async fn verify_and_update_state(
        ...
        let res = account
            ...
            .send()
            .await
            .map_err(|e| Error::Starknet(format!("Transaction send failed: {e}")))?;
